...
- **Edit mode hidden by default** — Agent mode now handles everything edit mode can do; edit mode is hidden from the agent picker by default (`setting(chat.editMode.hidden)`). Ask mode is now backed by a custom agent definition.
...
- **Agentic browser tools (Experimental)** — Agents can read and interact with the integrated browser using tools like `openBrowserPage`, `readPage`, `screenshotPage`, `clickElement`, `typeInPage`, and `runPlaywrightCode`. Enable with `setting(workbench.browser.enableChatTools:true)`.
...
- **Explore subagent for codebase search** — The Plan agent delegates codebase research to a dedicated read-only Explore subagent running on fast models. Configurable via `setting(chat.exploreAgent.defaultModel)`.
...
- **Contextual tips (Experimental)** — Tips in the Chat view help discover features tailored to your usage patterns (`setting(chat.tips.enabled)`).
- **Custom thinking phrases** — Customize loading text during reasoning/tool calls via `setting(chat.agent.thinking.phrases)`.
- **Collapsible terminal tool calls** — Terminal tool invocations displayed as collapsible sections to reduce visual noise (`setting(chat.tools.terminal.simpleCollapsible)`).
...
- **Inline chat hover mode** — New hover-based UI for inline chat (`setting(inlineChat.renderMode:hover)`).
- **Inline chat affordance** — Selection-triggered affordance for starting inline chat in the editor or gutter (`setting(inlineChat.affordance)`).
