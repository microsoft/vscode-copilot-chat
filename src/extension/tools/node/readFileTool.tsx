...
import { SessionReferenceScheme } from '../../prompt/common/chatVariablesCollection';
...
export class ReadFileTool implements ICopilotTool<ReadFileParams> {
	...
	private async getSnapshot(uri: URI) {
		...
		if (uri.scheme === 'copilot-skill' && uri.path.includes('/troubleshoot/')) {
			const sessionRefs = this._promptContext?.request?.references?.filter(ref => URI.isUri(ref.value) && ref.value.scheme === SessionReferenceScheme) ?? [];
