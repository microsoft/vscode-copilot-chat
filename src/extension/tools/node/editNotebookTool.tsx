...
import { findCell, findNotebook, isJupyterNotebookUri } from '../../../util/common/notebooks';
	...
	private validateInput({ editType, cellId, newCode }: { editType: 'edit' | 'insert' | 'delete'; cellId: string; newCode: string | undefined }, notebook: vscode.NotebookDocument) {
		...
		switch (editType) {
			case 'insert':
				...
				if (newCode.length && isJupyterNotebookUri(notebook.uri)) {
					if (newCode.startsWith('{') && newCode.includes('"cells') && newCode.includes('"cell_type') && newCode.includes('"source') && newCode.endsWith('}')) {
