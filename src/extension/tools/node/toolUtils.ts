...
export async function assertFileOkForTool(accessor: ServicesAccessor, uri: URI): Promise<void> {
	...
	const systemContextService = accessor.get(ISystemContextService);

	await assertFileNotContentExcluded(accessor, uri);

	const isSystemPath = systemContextService.isSystemPath(uri);

	if (!isSystemPath && !workspaceService.getWorkspaceFolder(normalizePath(uri)) && !customInstructionsService.isExternalInstructionsFile(uri) && uri.scheme !== Schemas.untitled) {
