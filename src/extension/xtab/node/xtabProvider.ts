...
export class XtabProvider implements IStatelessNextEditProvider {
	...
	private async doGetNextEditWithSelection(
		...
		const taggedCurrentFileContentResult = constructTaggedFile(
			...
			{ includeLineNumbers: { areaAroundCodeToEdit: false, currentFileContent: promptOptions.promptingStrategy === PromptingStrategy.XtabAggresivity } }
		);
		...
		const aggresivityLevel = this.configService.getExperimentBasedConfig<xtabPromptOptions.AggresivityLevel | undefined>(ConfigKey.TeamInternal.InlineEditsXtabAggresivityLevel, this.expService) ?? xtabPromptOptions.AggresivityLevel.Medium;
	...
	private pickSystemPrompt(promptingStrategy: xtabPromptOptions.PromptingStrategy | undefined): string {
		switch (promptingStrategy) {
			...
			case xtabPromptOptions.PromptingStrategy.XtabAggresivity:
