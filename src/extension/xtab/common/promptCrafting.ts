...
import { AggresivityLevel, CurrentFileOptions, DiffHistoryOptions, PromptingStrategy, PromptOptions } from '../../../platform/inlineEdits/common/dataTypes/xtabPromptOptions';
...
export class PromptPieces {
	constructor(
		...
		public readonly aggresivityLevel: AggresivityLevel,
...
export function getUserPrompt(promptPieces: PromptPieces): string {
	...
	const { activeDoc, xtabHistory, taggedCurrentDocLines, areaAroundCodeToEdit, langCtx, aggresivityLevel, computeTokens, opts } = promptPieces;
	...
	const postScript = promptPieces.opts.includePostScript ? getPostScript(opts.promptingStrategy, currentFilePath, aggresivityLevel) : '';
...
function getPostScript(strategy: PromptingStrategy | undefined, currentFilePath: string, aggresivityLevel: AggresivityLevel): string {
	...
	switch (strategy) {
		...
		case PromptingStrategy.Xtab275:
			...
			break;
		case PromptingStrategy.XtabAggresivity:
			postScript = `<|aggressive|>${aggresivityLevel}<|/aggressive|>`;
			break;
