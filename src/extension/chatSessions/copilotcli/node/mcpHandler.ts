...
export interface ICopilotCLIMCPHandler {
	...
	loadMcpConfig(uri?: URI): Promise<{ mcpConfig: Record<string, MCPServerConfig> | undefined; disposable: IDisposable }>;
}
...
export class CopilotCLIMCPHandler implements ICopilotCLIMCPHandler {
	...
	public async loadMcpConfig(uri?: URI): Promise<{ mcpConfig: Record<string, MCPServerConfig> | undefined; disposable: IDisposable }> {
		...
		if (isSessionsWindow) {
			return this.loadMcpConfigWithGateway(uri);
		}
	 ...
	 */
	private async loadMcpConfigWithGateway(uri?: URI): Promise<{ mcpConfig: Record<string, MCPServerConfig> | undefined; disposable: IDisposable }> {
		const mcpConfig: Record<string, MCPServerConfig> = {};
		const disposable = new DisposableStore();
		disposable.add({
			dispose: () => {
				this.logService.trace('[CopilotCLIMCPHandler] Disposing MCP gateway and servers');
			}
		});
		try {
			const gateway = await this.mcpService.startMcpGateway(uri ?? URI.from({ scheme: 'copilot-cli', path: `mcp-gateway-${generateUuid()}` }));
			if (gateway) {
