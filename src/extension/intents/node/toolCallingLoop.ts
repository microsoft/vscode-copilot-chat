...
export abstract class ToolCallingLoop<TOptions extends IToolCallingLoopOptions = IToolCallingLoopOptions> extends Disposable {
	 ...
	 */
	protected async executeStopHook(input: StopHookInput, outputStream: ChatResponseStream | undefined, sessionId: string, token: CancellationToken | PauseController): Promise<StopHookResult> {
		try {
	 ...
	 */
	public async runStartHooks(token: CancellationToken | PauseController): Promise<void> {
