...
export function formatHookErrorMessage(errorMessage: string): string {
	if (errorMessage) {
		return l10n.t('A hook failed with fatal error. Please check the Hooks output channel for more details. Error message: {0}', errorMessage);
	}
	return l10n.t('A hook failed with fatal error. Please check the Hooks output channel for more details.');
}
