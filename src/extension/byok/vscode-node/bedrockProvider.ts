...
export class BedrockBYOKModelRegistry implements BYOKModelRegistry {
        ...
        async getAllModels(): Promise<{ id: string; name: string }[]> {
                ...
                return Object.entries(this._knownModels).map(([id, info]) => ({ id, name: info.name }));
        }
                ...
                if (tools.length > 0) {
                        ...
                        if (options.toolChoice) {
                                ...
                                if (options.toolChoice === 'auto') {
                                        ...
                                        toolChoice = { tool: { name: options.toolChoice.name } };
                                }
                                if (toolChoice) {
                        ...
                        for await (const event of (response.stream as AsyncIterable<ConverseStreamOutput>) ?? []) {
                                ...
                                if (toolStopIndex !== undefined) {
                                        ...
                                        if (pending) {
                                                ...
                                                wrappedProgress.report({ index: 0, part: new LanguageModelToolCallPart(pending.name, args, pending.id) });
