		...
		if (toolResult === undefined) {
			...
			if (toolResult === undefined) {
				try {
					...
					sendInvokedToolTelemetry(this.promptEndpoint.acquireTokenizer(), this.telemetryService, this.props.toolCall.name, this.props.toolCall.arguments, toolResult);
				} catch (err) {
...
export function sendInvokedToolTelemetry(tokenizer: ITokenizer, telemetry: ITelemetryService, toolName: string, toolInput: string, toolResult: LanguageModelToolResult2) {
	Promise.all([
		...
		tokenizer.tokenLength(toolInput)
	]).then(([{ tokenCount: responseTokenCount }, inputTokenCount]) => {
		/* __GDPR__
			"agent.tool.responseLength" : {
				"owner": "connor4312",
				"comment": "Counts the number of tokens generated by tools",
				"toolName": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "The name of the tool being invoked." },
				"tokenCount": { "classification": "SystemMetaData", "purpose": "PerformanceAndHealth", "comment": "Number of tokens in the tool response.", "isMeasurement": true },
				"inputTokenCount": { "classification": "SystemMetaData", "purpose": "PerformanceAndHealth", "comment": "Number of tokens in the tool input.", "isMeasurement": true }
			}
		*/
		telemetry.sendMSFTTelemetryEvent('agent.tool.responseLength', { toolName }, { tokenCount: responseTokenCount, inputTokenCount });
	});
}
