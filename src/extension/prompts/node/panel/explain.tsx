...
import { CurrentChange, CurrentChangeInput } from '../feedback/currentChange';
...
export class ExplainPrompt extends PromptElement<ExplainPromptProps, ExplainPromptState> {
	...
	override async prepare() {
		...
		const { document, selection, changeInput } = this.props;
		if (changeInput === undefined && document?.uri && selection) {
	...
	override render(state: ExplainPromptState, sizing: PromptSizing): PromptPiece<any, any> | undefined {
		...
		const isExplainingChanges = this.props.changeInput !== undefined;
		const contextDescription = isExplainingChanges ? 'code selection or change context' : 'code selection';
		const answerInstruction = isExplainingChanges
			? 'If the user provided a specific question or error, answer it based on the provided code or change context. Otherwise focus on explaining the selected code or current changes.'
			: 'If the user provided a specific question or error, answer it based on the selected code and additional provided context. Otherwise focus on explaining the selected code.';
			...
			<>
				...
				<HistoryWithInstructions inline={this.props.isInlineChat} historyPriority={600} passPriority history={history}>
					<InstructionMessage priority={1000}>
						...
						1. Examine the provided {contextDescription} and any other context like user question, related errors, project details, class definitions, etc.<br />
						2. If you are unsure about the code, concepts, or the user's question, ask clarifying questions.<br />
						3. {answerInstruction}<br />
				...
				{isExplainingChanges
					? <UserMessage priority={900}>
						...
						<CurrentChange input={this.props.changeInput ?? []} logService={this.props.logService} priority={900} />
					</UserMessage>
					: <>
