...
export class Gpt54LargePromptExp extends PromptElement<DefaultAgentPromptProps> {
	...
	constructor(
		...
		super(props);
		Gpt54LargePromptExp.isEnabled = this.configurationService.getExperimentBasedConfig(ConfigKey.EnableGpt54LargePromptExp, this.experimentationService);
	}

	async render(state: void, sizing: PromptSizing) {
		const isEnabled = Gpt54LargePromptExp.isEnabled;
		if (!isEnabled) {
			return undefined;
		}
...
export class Gpt54LargePromptExpReminderInstructions extends PromptElement<ReminderInstructionsProps> {
	async render(state: void, sizing: PromptSizing) {
		return <>
