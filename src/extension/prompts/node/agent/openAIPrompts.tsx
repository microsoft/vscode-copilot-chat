...
class HiddenModelEPrompt extends PromptElement<DefaultAgentPromptProps> {
	async render(state: void, sizing: PromptSizing) {
		const tools = detectToolCapabilities(this.props.availableTools);
		return <InstructionMessage>
			<Tag name='coding_agent_instructions'>
...
class ModelEPromptResolver implements IAgentPrompt {
	...
	static async matchesModel(endpoint: IChatEndpoint): Promise<boolean> {
		return isHiddenModelE(endpoint);
	}

	static readonly familyPrefixes = [];

	resolvePrompt(endpoint: IChatEndpoint): PromptConstructor | undefined {
		return HiddenModelEPrompt;
	}
}

PromptRegistry.registerPrompt(OpenAIPromptResolver);
PromptRegistry.registerPrompt(ModelBPromptResolver);
PromptRegistry.registerPrompt(ModelEPromptResolver);
