...
class ConversationHistorySummarizer {
	...
	private async getSummaryWithFallback(propsInfo: ISummarizedConversationHistoryInfo): Promise<FetchSuccess<string>> {
		const forceMode = this.configurationService.getConfig<string | undefined>(ConfigKey.AdvancedExperimental.AgentHistorySummarizationMode);
	...
	private async getSummary(mode: SummaryMode, propsInfo: ISummarizedConversationHistoryInfo): Promise<FetchSuccess<string>> {
		...
		const forceGpt41 = this.configurationService.getExperimentBasedConfig(ConfigKey.AdvancedExperimentalExperiments.AgentHistorySummarizationForceGpt41, this.experimentationService);
		...
		const promptCacheMode = this.configurationService.getExperimentBasedConfig(ConfigKey.AdvancedExperimentalExperiments.AgentHistorySummarizationWithPromptCache, this.experimentationService);
		try {
