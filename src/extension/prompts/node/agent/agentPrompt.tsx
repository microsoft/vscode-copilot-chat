...
class AgentTasksInstructions extends PromptElement {
	...
	render() {
		...
		return <>
			The following tasks can be executed using the {ToolName.RunTaskCore} or {ToolName.RunTask} tool if they are not already running:<br />
					...
					{tasks.map((t, i) => {
						...
						return (
							<Tag name='task' attrs={{ id: `${t.type}: ${t.label || i}` }}>
								...
								{isActive && <> (This task is currently running. You can use the {ToolName.GetTaskOutputCore} or {ToolName.GetTaskOutput} tool to view its output.)</>}
							</Tag>
						);
					})}
				</Tag>
