...
import type { ChatHookCommand, ChatHookResult, ChatHookResultKind, ChatRequestHooks, ChatResponseStream, Uri } from 'vscode';
...
describe('PreToolUse hookProgress on deny', () => {
	...
	/**
	 * Runs the same onSuccess/onError callback logic as the real
	 * ChatHookService.executePreToolUseHook, using processHookResults.
	 */
	function runPreToolUseCallbacks(toolName: string, results: ChatHookResult[]): { mostRestrictiveDecision?: string; winningReason?: string } {
		let mostRestrictiveDecision: 'allow' | 'deny' | 'ask' | undefined;
		let winningReason: string | undefined;
		...
		processHookResults({
			...
			onSuccess: (output) => {
				...
				const hookOutput = output as { hookSpecificOutput?: IPreToolUseHookSpecificOutput };
				...
				if (decision && (mostRestrictiveDecision === undefined || (permissionPriority[decision] ?? 0) > (permissionPriority[mostRestrictiveDecision] ?? 0))) {
					...
					winningReason = hookSpecificOutput.permissionDecisionReason;
					if (decision === 'deny') {
						const messageWithTool = winningReason ? `Tried to use ${toolName} - ${winningReason}` : `Tried to use ${toolName}`;
						mockStream.hookProgress('PreToolUse', formatHookErrorMessage(messageWithTool));
					}
				}
			},
		...
		return { mostRestrictiveDecision, winningReason };
	}

	it('shows hookProgress when success path returns permissionDecision deny', () => {
		const results: ChatHookResult[] = [
			hookResult({ hookSpecificOutput: { permissionDecision: 'deny', permissionDecisionReason: 'Security policy' } }),
		];

		const { mostRestrictiveDecision } = runPreToolUseCallbacks('run_terminal', results);
		expect(mostRestrictiveDecision).toBe('deny');
		expect(mockStream.hookProgressCalls).toHaveLength(1);
		expect(mockStream.hookProgressCalls[0].hookType).toBe('PreToolUse');
		expect(mockStream.hookProgressCalls[0].stopReason).toContain('Tried to use run_terminal - Security policy');
	});

	it('shows hookProgress when error path denies', () => {
		const results: ChatHookResult[] = [
			hookResult('hook blocked this tool', 'error'),
		];
		...
		expect(mockStream.hookProgressCalls).toHaveLength(1);
		expect(mockStream.hookProgressCalls[0].hookType).toBe('PreToolUse');
		expect(mockStream.hookProgressCalls[0].stopReason).toContain('Tried to use run_terminal - hook blocked this tool');
	});

	it('does not show hookProgress for allow decision', () => {
		const results: ChatHookResult[] = [
			hookResult({ hookSpecificOutput: { permissionDecision: 'allow', permissionDecisionReason: 'Tool is safe' } }),
		];

		runPreToolUseCallbacks('run_terminal', results);
		expect(mockStream.hookProgressCalls).toHaveLength(0);
	});

	it('shows hookProgress with tool name only when deny has no reason', () => {
		const results: ChatHookResult[] = [
			hookResult({ hookSpecificOutput: { permissionDecision: 'deny' } }),
		];

		runPreToolUseCallbacks('edit_file', results);
		expect(mockStream.hookProgressCalls).toHaveLength(1);
		expect(mockStream.hookProgressCalls[0].stopReason).toContain('Tried to use edit_file');
	});
});

describe('PostToolUse hookProgress on block', () => {
	let mockStream: MockChatResponseStream;
	let logService: TestLogService;
	...
	/**
	 * Runs the same onSuccess/onError callback logic as the real
	 * ChatHookService.executePostToolUseHook, using processHookResults.
	 */
	function runPostToolUseCallbacks(toolName: string, results: ChatHookResult[]): { hasBlock: boolean; blockReason?: string } {
		let hasBlock = false;
		let blockReason: string | undefined;
		...
		processHookResults({
			...
			onSuccess: (output) => {
				...
				const hookOutput = output as { decision?: string; reason?: string };
				if (hookOutput.decision === 'block' && !hasBlock) {
					hasBlock = true;
					blockReason = hookOutput.reason;
					const messageWithTool = blockReason ? `\nTried to use ${toolName}\n${blockReason}` : `Tried to use ${toolName}`;
					mockStream.hookProgress('PostToolUse', formatHookErrorMessage(messageWithTool));
				}
			},
			onError: (errorMessage) => {
				if (!hasBlock) {
					...
					const messageWithTool = errorMessage ? `\nTried to use ${toolName}\n${errorMessage}` : `Tried to use ${toolName}`;
					mockStream.hookProgress('PostToolUse', undefined, formatHookErrorMessage(messageWithTool));
				}
			},
		});

		return { hasBlock, blockReason };
	}

	it('shows hookProgress when success path returns block decision', () => {
		const results: ChatHookResult[] = [
			hookResult({ decision: 'block', reason: 'Lint errors found' }),
		];

		const { hasBlock } = runPostToolUseCallbacks('run_terminal', results);
		expect(hasBlock).toBe(true);
		expect(mockStream.hookProgressCalls).toHaveLength(1);
		expect(mockStream.hookProgressCalls[0].hookType).toBe('PostToolUse');
		expect(mockStream.hookProgressCalls[0].stopReason).toContain('Tried to use run_terminal\nLint errors found');
	});

	it('shows hookProgress when error path blocks', () => {
		const results: ChatHookResult[] = [
			hookResult('hook errored', 'error'),
		];
	...
	it('does not show hookProgress when no block decision', () => {
		const results: ChatHookResult[] = [
			hookResult({ hookSpecificOutput: { additionalContext: 'some context' } }),
		];

		runPostToolUseCallbacks('run_terminal', results);
		expect(mockStream.hookProgressCalls).toHaveLength(0);
	});

	it('shows hookProgress with tool name only when block has no reason', () => {
		const results: ChatHookResult[] = [
			hookResult({ decision: 'block' }),
		];

		runPostToolUseCallbacks('edit_file', results);
		expect(mockStream.hookProgressCalls).toHaveLength(1);
		expect(mockStream.hookProgressCalls[0].stopReason).toContain('Tried to use edit_file');
	});
});
