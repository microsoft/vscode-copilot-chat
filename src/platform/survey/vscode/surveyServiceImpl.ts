...
export class SurveyService implements ISurveyService {
	...
	public async checkInactiveUserHeuristic(): Promise<boolean> {
		...
		if (isEligible) {
			const sessionProbability = this.experimentationService.getTreatmentVariable<number>('vscode', 'copilotchat.feedback.sessionProbability.inactive') ?? DEFAULT_SESSION_PROBABILITY_INACTIVE;
	...
	private async checkEligibility(): Promise<boolean> {
		...
		if (isEligible) {
			const sessionProbability = this.experimentationService.getTreatmentVariable<number>('vscode', 'copilotchat.feedback.sessionProbability') ?? DEFAULT_SESSION_PROBABILITY;
			if (this.sessionSeed < sessionProbability / 100) {
				const notificationProbability = this.experimentationService.getTreatmentVariable<number>('vscode', 'copilotchat.feedback.notificationProbability') ?? DEFAULT_NOTIFICATION_PROBABILITY;
