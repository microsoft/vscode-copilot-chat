...
export class MockCustomInstructionsService implements ICustomInstructionsService {
	...
	setExtensionSkillInfos(infos: { uri: URI; skillName: string; skillFolderUri: URI; extensionId: string }[]): void {
		...
		infos.forEach(info => this.extensionSkillInfos.set(info.uri.toString(), { skillName: info.skillName, skillFolderUri: info.skillFolderUri, extensionId: info.extensionId, storage: SkillStorage.Extension }));
	}
