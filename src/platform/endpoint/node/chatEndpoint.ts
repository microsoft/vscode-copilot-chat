...
export class ChatEndpoint implements IChatEndpoint {
	...
	public async _makeChatRequest2(options: IMakeChatRequestOptions, token: CancellationToken) {
		return this._chatMLFetcher.fetchOne({
			requestOptions: {},
			...options,
			delegate: this.useResponsesApi ? this._instantiationService.createInstance(ResponsesApiDelegate, this.model, this._modelMetadata, this.urlOrRequestMetadata) : this,
			endpoint: this,
		}, token);
	}
...
class DefaultChatRequestDelegate implements IChatRequestDelegate {
	constructor(
