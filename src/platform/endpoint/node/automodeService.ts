...
export class AutomodeService extends Disposable implements IAutomodeService {
	...
	private async _tryRouterSelection(
		...
		try {
			...
			const result = await this._routerDecisionFetcher.getRouterDecision(prompt, token.session_token, token.available_models, undefined, contextSignals, conversationId);
