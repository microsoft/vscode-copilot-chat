...
import { loadAndParseInput } from './pipeline/parseInput';
import { processAllRows, parseSuggestedEdit } from './pipeline/replayRecording';
import { generatePromptFromRecording, IGeneratedPrompt } from './pipeline/promptStep';
import { generateResponse, generateAllResponses, IResponseGenerationInput } from './pipeline/responseStep';
import { assembleSample, writeSamples, resolveOutputPath, ISample } from './pipeline/output';
...
async function run(opts: SimulationOptions): Promise<RunResult> {
	...
	switch (true) {
		...
		case opts.listModels:
			...
			return;
		case !!opts.input:
			if (opts.parallelism > 1 && !opts.inputWorkerMode) {
...
async function runInputPipeline(opts: SimulationOptions): Promise<void> {
	const inputPath = opts.input!;
	const strategy = resolvePromptingStrategy(opts.inputStrategy ?? 'patchBased02');
	const verbose = !!opts.verbose;
	const concurrency = opts.parallelism;
	const rowOffset = opts.inputRowOffset;
	...
	// Step 1: Parse input
	const { rows, errors } = await loadAndParseInput(inputPath);
	...
	await runWithConcurrency(processed, concurrency, async (p, i) => {
		const result = await generatePromptFromRecording(testAccessor, p.recordingInfo);
		if ('error' in result) {
			promptErrors.push({ index: i, error: `[sample ${i + rowOffset}, ${p.row.activeDocumentLanguageId}, ${p.activeFilePath}] ${result.error}` });
		} else {
	...
	const outputPath = resolveOutputPath(inputPath, opts.inputOutput);
	const samples = [];
 ...
 */
async function runInputPipelineParallel(opts: SimulationOptions): Promise<void> {
	const inputPath = opts.input!;
	const verbose = !!opts.verbose;

	const contents = await fs.promises.readFile(inputPath, 'utf8');
	const records = JSON.parse(contents) as unknown[];
	const numWorkers = Math.min(os.cpus().length, opts.parallelism, Math.ceil(records.length / 25));
	...
	try {
		...
		for (let w = 0; w < numWorkers; w++) {
			...
			const args = [
				'--input', chunkPath,
				'--strategy', opts.inputStrategy ?? 'patchBased02',
				'--out', resultPath,
